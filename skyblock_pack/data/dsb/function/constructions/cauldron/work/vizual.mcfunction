...
$data modify storage dsb:cts Temp.count set from storage dsb:cts Cauldron.$(out).itemCount
data modify storage dsb:cts Temp.uuid set from storage gu:main out

execute positioned ~ ~1.65 ~ summon minecraft:item_display run function dsb:constructions/cauldron/work/on_item with storage dsb:cts Temp
