...
execute store result score *var1 variables run random value 600..1800 dsb:get_time_for_l.task
...
###
function dsb:constructions/huge_library/task/work/delay with storage dsb:cts Temp


#Если нет игроков, то ничего не делаем
execute positioned ~ ~0.5 ~ positioned ~-2.5 ~-2.5 ~-2.5 unless entity @a[dx=4, dy=4, dz=4] run return 0
