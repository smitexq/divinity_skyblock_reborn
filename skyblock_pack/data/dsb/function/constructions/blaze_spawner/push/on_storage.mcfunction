...
setblock 17112022 1 0 minecraft:light_blue_shulker_box{LootTable:"dsb:items/fire_soul"}
...
$execute store success score *var1 variables run data modify storage dsb:cts BlazeSpawner.$(out).itemCount set from block 17112022 0 0 Items[0].count
$execute if score *var1 variables matches 0 run data modify storage dsb:cts BlazeSpawner.$(out).itemCount set value 0
###
...
#Виузал
$execute as $(out) at @s run function dsb:constructions/blaze_spawner/work/vizual with storage gu:main