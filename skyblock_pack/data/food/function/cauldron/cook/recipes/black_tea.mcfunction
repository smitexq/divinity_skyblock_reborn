...
data modify storage dsb:recipes TitleText.text1 set value "dsb.items.food.black_tea"
data modify storage dsb:recipes TitleText.text2 set value "item.minecraft.sugar"
execute if score *var1 variables matches ..3 run return run function food:cauldron/cook/recipes/title with storage dsb:recipes TitleText
...
#Черный чай
execute store result storage dsb:recipes TitleText.count int 1 store result score *var1 variables run data get storage dsb:recipes Tea.Items[{components:{"minecraft:custom_data":{id:"black_tea_leaves"}}}].count

scoreboard players set *var2 variables 7
execute store result storage dsb:recipes TitleText.count int 1 run scoreboard players operation *var2 variables -= *var1 variables
data modify storage dsb:recipes TitleText.text2 set value "dsb.items.food.black_tea_leaves"
