#Записываем количество предмета
execute store result storage dsb:recipes TitleText.count int 1 store result score *var1 variables run data get storage dsb:recipes Tea.Items[{id:"minecraft:oxeye_daisy"}].count
#Сколько должно быть
...
data modify storage dsb:recipes TitleText.text1 set value "dsb.items.food.oxeye_daisy_tea"
data modify storage dsb:recipes TitleText.text2 set value "block.minecraft.oxeye_daisy"
#Если меньше нужного, то выводим сообщение о нехватке
