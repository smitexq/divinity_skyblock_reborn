...
###Сначала проверяем, что интеракт должен висеть тег завершения готовки. Если его нет, то не проверяем, иначе выводим сообщение игроку
$execute as $(out) if entity @s[tag=!confirm] run return 0

scoreboard players set #stage CookingTea 2
title @s actionbar {"translate":"dsb.events.tea_cauldron.take_tea","color":"gold"}