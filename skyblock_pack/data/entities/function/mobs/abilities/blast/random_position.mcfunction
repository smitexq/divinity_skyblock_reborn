...
execute unless entity @s[distance=4..] if score @s RecursionDepth matches ..100 run return run function entities:mobs/abilities/blast/random_position
...
execute at @s if entity @e[type=marker,tag=ExposionMarker,distance=..9] if score @s RecursionDepth matches ..100 run scoreboard players set *var2 variables -1
execute if score *var2 variables matches -1 run return run function entities:mobs/abilities/blast/random_position

#Тег, помечающий маркер как постоянный, чтобы для новых маркеров можно было проверять, чтобы рядом не было уже постоянных
tag @s add ExposionMarker


#Если маркеров меньше трех, то спавним еще. И из-за return функция ниже не выполняется
execute if score *var1 variables matches ..2 run return run function entities:mobs/abilities/blast/on_marker

#Последнему маркеру выдается тег, чтобы призвать возгорание
tag @s add last

#А если маркеров 2, то return не срабатывает и мы выполняем команду ниже (разрушаем блоки на месте маркеров)
execute as @e[type=marker,tag=ExposionMarker] at @s run function entities:mobs/abilities/blast/end
